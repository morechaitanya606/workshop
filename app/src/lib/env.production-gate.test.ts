...
describe("production env gate", () => {
    ...
    it("still demands the build-time secrets everywhere, preview included", async () => {
        // Those are baked into the bundle or used during prerender, so a preview genuinely
        // cannot do without them. The relaxation above must not have widened to these.
        const { getMissingProductionEnvVars } = await loadEnvWith({
            NODE_ENV: "production",
            VERCEL_ENV: "preview",
