...
import { ArrowLeft, Loader2, Check, Crop, Upload } from "lucide-react";
...
export default function AdminEditWorkshopPage() {
    ...
    const coverPreviewEntry = form.coverImage.trim();
    const coverPreview = isSupportedWorkshopImageUrl(coverPreviewEntry) ? coverPreviewEntry : "";
    const galleryPreviewEntries = toList(form.galleryImages);
    const galleryPreviews = galleryPreviewEntries.filter((src) => isSupportedWorkshopImageUrl(src));
    const skippedGalleryPreviewCount = galleryPreviewEntries.length - galleryPreviews.length;
    const locationPreviewEntries = toList(form.locationImages);
    const locationPreviews = locationPreviewEntries.filter((src) =>
        isSupportedWorkshopImageUrl(src)
    );
    const skippedLocationPreviewCount = locationPreviewEntries.length - locationPreviews.length;
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="md:col-span-2">
                            ...
                            {coverPreviewEntry && !coverPreview && (
                                <p className="mt-2 text-xs font-inter text-amber-700">
                                    This URL is not an image link, so the preview is hidden.
                                </p>
                            )}
                        </div>
                        ...
                        <div className="md:col-span-2">
                            ...
                            {skippedGalleryPreviewCount > 0 && (
                                <p className="mt-2 text-xs font-inter text-amber-700">
                                    {skippedGalleryPreviewCount} gallery URL
                                    {skippedGalleryPreviewCount === 1 ? "" : "s"} not previewed
                                    because it is not an image link.
                                </p>
                            )}
                        </div>
                        ...
                        <div className="md:col-span-2">
                            ...
                            {skippedLocationPreviewCount > 0 && (
                                <p className="mt-2 text-xs font-inter text-amber-700">
                                    {skippedLocationPreviewCount} location URL
                                    {skippedLocationPreviewCount === 1 ? "" : "s"} not previewed
                                    because it is not an image link.
                                </p>
                            )}
                        </div>
