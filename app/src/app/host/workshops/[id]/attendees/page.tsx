...
export default function AttendeesPage({ params }: { params: { id: string } }) {
    ...
    const toggleCheckIn = async (bookingId: string, currentStatus: boolean) => {
        ...
        // Optimistic update
        setAttendees((prev) =>
            prev.map((a) => (a.id === bookingId ? { ...a, attended: newStatus } : a))
        );
        ...
        try {
            ...
            // Revert on failure
            setAttendees((prev) =>
                prev.map((a) => (a.id === bookingId ? { ...a, attended: currentStatus } : a))
            );
    ...
    const content = (
        <>
            <div className="mb-8">
                <Link
                    href="/host/workshops"
                    className="inline-flex items-center gap-2 text-sm text-dark-muted hover:text-terracotta transition-colors mb-4"
                >
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-gray-100 bg-cream-50">
                                <th className="p-4 text-xs font-inter font-semibold uppercase text-dark-muted tracking-wider">
                                    Attendee
                                </th>
                                <th className="p-4 text-xs font-inter font-semibold uppercase text-dark-muted tracking-wider">
                                    Contact
                                ...
                                <th className="p-4 text-xs font-inter font-semibold uppercase text-dark-muted tracking-wider text-right">
                                    Status
                                </th>
                            </tr>
                        </thead>
                            ...
                            {attendees.map((attendee) => (
                                ...
                                >
                                    <td className="p-4 align-middle">
                                        <p className="font-semibold text-dark font-inter text-sm">
                                    ...
                                    <td className="p-4 align-middle text-right">
                                        <button
                                            onClick={() =>
                                                toggleCheckIn(attendee.id, attendee.attended)
                                            }
