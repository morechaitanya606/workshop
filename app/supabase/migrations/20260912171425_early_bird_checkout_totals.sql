-- Validate early-bird discounts inside atomic booking confirmation.
...
create or replace function public.confirm_booking_from_hold(
    ...
    p_early_bird_discount integer default 0
)
returns uuid
language plpgsql
security invoker
...
declare
    ...
    v_expected_early_bird integer := 0;
    v_requested_early_bird integer := greatest(coalesce(p_early_bird_discount, 0), 0);
    v_expected_discount integer := 0;
    v_requested_discount integer := greatest(coalesce(p_discount_applied, 0), 0);
    v_subtotal_after_early_bird integer;
    v_subtotal integer;
    v_total integer;
    v_booking_id uuid;
begin
    ...
    if coalesce(v_workshop.early_bird_enabled, false)
       and coalesce(v_workshop.early_bird_discount_value, 0) > 0
       and coalesce(v_workshop.early_bird_days_after_listing, 0) > 0
       and v_workshop.created_at is not null
       and ceil(extract(epoch from (date_trunc('day', now()) - date_trunc('day', v_workshop.created_at))) / 86400)
           <= v_workshop.early_bird_days_after_listing then
        if v_workshop.early_bird_discount_type = 'fixed' then
            v_expected_early_bird := greatest(coalesce(v_workshop.early_bird_discount_value, 0), 0) * v_hold.guests;
        else
            v_expected_early_bird := floor(
                (v_original_subtotal::numeric * greatest(coalesce(v_workshop.early_bird_discount_value, 0), 0)::numeric) / 100
            )::integer;
        end if;
        v_expected_early_bird := least(v_original_subtotal, greatest(v_expected_early_bird, 0));
    end if;

    if v_requested_early_bird <> v_expected_early_bird then
        raise exception 'EARLY_BIRD_DISCOUNT_MISMATCH';
    end if;

    v_subtotal_after_early_bird := v_original_subtotal - v_expected_early_bird;
    ...
    if p_coupon_id is not null then
        ...
        if v_coupon.min_order_amount is not null and v_subtotal_after_early_bird < v_coupon.min_order_amount then
        ...
        if v_coupon.discount_type = 'percentage' then
            v_expected_discount := round(
                (v_subtotal_after_early_bird::numeric * greatest(coalesce(v_coupon.discount_value, 0), 0)::numeric) / 100
            )::integer;
        ...
        v_expected_discount := least(v_subtotal_after_early_bird, greatest(v_expected_discount, 0));
    ...
    v_subtotal := coalesce(p_subtotal, v_subtotal_after_early_bird - v_expected_discount);
    v_total := coalesce(p_total, v_subtotal + v_service_fee);

    if v_subtotal <> v_subtotal_after_early_bird - v_expected_discount then
